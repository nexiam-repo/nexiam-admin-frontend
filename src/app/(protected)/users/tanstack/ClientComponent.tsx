...
export default function ClientComponent() {
  ...
  if (errorLoadingTodos)
    return <p>Error loading todos: {loadError?.message}</p>;
  ...
  return (
    <div>
      ...
      >
        ...
        {createTodo.isError && (
          <p>Error creating: {createTodo.error?.message}</p>
        )}
      </form>
      ...
      <ul>
        {todos!.map((todo) => (
          <li key={todo.id} style={{ marginBottom: '1em' }}>
            ...
            {updateTodo.isError && (
              <p style={{ color: 'red' }}>
                Update error: {updateTodo.error?.message}
              </p>
            )}
            {deleteTodo.isError && (
              <p style={{ color: 'red' }}>
                Delete error: {deleteTodo.error?.message}
              </p>
            )}
          </li>
        ))}
